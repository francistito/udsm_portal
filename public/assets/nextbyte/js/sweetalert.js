...
function addGetForms() {
    $("[data-method='get']").append(function () {
        if (!$(this).find('form').length > 0) {
            return "\n<form action='" + $(this).attr('href') + "' method='GET' name='get_item' style='display:none'>\n" +
                "<input type='hidden' name='_method' value='" + $(this).attr('data-method') + "'>\n" +
                "<input type='hidden' name='_token' value='" + $('meta[name="csrf-token"]').attr('content') + "'>\n" +
                '</form>\n';
        } else { return '' }
...
$(function () {
     ...
     * Generic confirm form delete using Sweet Alert
     */
    $('body').on('submit', 'form[name=get_item]', function (e) {
    ...
    }).on('click', 'a[name=get_item]', function (e) {
        /**
         * Generic 'are you sure' confirm box
         */
        e.preventDefault();
