...
trait UserAccess
{
    ...
    public function hasRoleId($nameOrId) {
        foreach ($this->roles as $role) {
            //First check to see if it's an ID
            if (is_numeric($nameOrId)) {
...
//                        //First check to see if it's an ID
    ...
    public function hasWorkflowModuleDefinitionByWfDef($wf_definition_id)
    {
        $wf_definition = WfDefinition::query()->find($wf_definition_id);
        $level = $wf_definition->level;
        $wf_module_group_id = $wf_definition->wfModule->wf_module_group_id;
        $type = $wf_definition->wfModule->type;
        $workflow = new Workflow(['wf_module_group_id' => $wf_module_group_id, 'type' => $type]);
        return $workflow->userHasAccess($this->id, $level);
    }
